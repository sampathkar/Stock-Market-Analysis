...
with open ("share_list.txt", "w") as o:
  o.write("Following Stocks meets the selection Criteria: \n\n")
...
all_shares_list2 = ['ASIA', 'ASSO', 'BROW', 'DIAO', 'CEYL', 'CDBF', 'COMN','BERU', 'EXPO', 'HAYL', 'HUNT', 'HVAF', 'HYDP', 'IDL', 'JINS', 'LALU', 'LOLF', 'LWL', 'LGGL','MWEL', 'MSL', 'MULT', 'LOLD', 'NEH', 'ODEL', 'PANA', 'PEOP','RAIG', 'RFLL', 'RAFL', 'RHL', 'SANA', 'SIFI', 'SOCL', 'SOFT', 'AMBE', 'TEEJ','UNIO', 'UCHE', 'VALL', 'VALI', 'CITW', 'LANK', 'GOOD', 'AFSL', 'ALUM', 'ABLT', 'HPWR','CITH', 'ASHO', 'BANS', 'PRIE', 'SLND', 'MHDL', 'SFL', 'CAPR']
            ...
            if last_price >= mov10 >= mov20 >= mov50 >= mov100 and last_price >= mov200:#condition 1
              ...
              if rsi >= 65:#condition 2
                ...
                if (float(fiftytwo_high)*.75)<= last_price and (float(fiftytwo_low)*1.3)<=last_price:#condition 3
                  ...
                  if rsi >= 70 and tech_ind_data.iloc[1, 2] == tech_ind_data.iloc[3, 2] == tech_ind_data.iloc[9, 2] == tech_ind_data.iloc[1, 2] == "buy":
                    with open ("share_list.txt", "a") as o:
                      o.write(f"🔥 ALSO Technical Indicators Say 'BUY' {x} 🔥\n\n")
                    volume = stock_info.iloc[0, 7]
                    avg_volume = stock_info.iloc[0, 10]
                    if volume / avg_volume >= 3:
                      with open ("share_list.txt", "a") as o:
                        o.write(f"🔥🔥 more than 300% increase in the volume in {x} 🔥🔥\n\n\n")
